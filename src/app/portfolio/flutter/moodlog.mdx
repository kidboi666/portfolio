...
> AI 기반 감정 일기 - 당신의 감정을 기록하고 위로받으세요.
...
<ResponsiveGrid>
  ...
  - MoodLog는 사용자의 일상적인 감정을 기록하고 AI가 맞춤형 위로 메시지를 제공하는 크로스플랫폼 모바일 앱입니다. 로그인 없이 완전한 프라이버시를 보장하며, 로컬 우선 아키텍처로 설계되어 모든 데이터가 사용자 기기에 안전하게 저장됩니다.
</ResponsiveGrid>
---
<ResponsiveGrid>
  ### Deployment Status
...
<ResponsiveGrid>
  ...
  - 1인 개발 (기획, 디자인, 개발, 배포, 운영)
</ResponsiveGrid>
<ResponsiveGrid>
  ### Project Links

  - **GitHub**: [github.com/kidboi666/moodlog](https://github.com/kidboi666/moodlog)
  - **App Store**: [MoodLog on iOS](https://apps.apple.com/kr/app/%EB%AC%B4%EB%93%9C%EB%A1%9C%EA%B7%B8/id6752590873)
  - **Play Store**: [MoodLog on Android](https://play.google.com/store/apps/details?id=com.kidboi666.moodlog)
</ResponsiveGrid>
...
<ResponsiveGrid>
  ...
  - **Flutter** 3.0+\
  크로스플랫폼 UI 프레임워크로 단일 코드베이스로 iOS/Android 앱 개발
  - **Dart** 3.0+\
  강력한 타입 시스템과 null safety로 안정적인 코드 작성
  - **Provider**\
  MVVM 패턴 기반 상태 관리로 UI와 비즈니스 로직 분리
  - **Go Router**\
  선언적 라우팅으로 딥링크 및 네비게이션 관리
  - **Freezed**\
  불변 데이터 클래스 자동 생성으로 보일러플레이트 코드 감소
</ResponsiveGrid>
---
<ResponsiveGrid>
  ### Backend & Services
  ...
  - **Drift (SQLite)**\
  타입 안전한 쿼리 작성 가능한 로컬 데이터베이스 ORM
  - **Firebase Gemini AI**\
  일기 내용을 분석하여 맞춤형 위로 메시지 생성
  - **Firebase Analytics**\
  사용자 행동 분석 및 데이터 드리븐 의사결정 지원
  - **Google Mobile Ads**\
  AdMob 통합으로 앱 수익화 구조 마련
</ResponsiveGrid>
---
<ResponsiveGrid>
  ...
  - **Clean Architecture**\
  계층 분리 및 의존성 관리로 테스트 가능하고 확장 가능한 구조 구축
  - **MVVM (Model-View-ViewModel)**\
  UI와 비즈니스 로직 완전 분리로 유지보수성 향상
  - **Repository Pattern**\
  데이터 소스 추상화로 로컬 DB와 외부 API 간 일관된 인터페이스 제공
  - **UseCase Pattern**\
  비즈니스 로직 캡슐화로 단일 책임 원칙 준수
</ResponsiveGrid>
...
<ResponsiveGrid>
  ...
  <div>
    **문제**
    - StatefulWidget의 setState와 ViewModel 패턴 혼재 사용
...
<ResponsiveGrid>
  ...
  <div>
    **문제**
    - 도메인/데이터 계층에서 Flutter UI 의존성 참조
    - 프레임워크 독립성 위배
    - 레이어 간 경계 모호
